...
\renewcommand{\coverletter} {
  ...
  keen interest in computational Physics and making programs
  faster. I've worked on multiple projects throughout my university
  years, in a variety of disciplines, including Graph theory
  (modelling traffic + neural network based predictive exploration),
  Bayesian inference using Nested Sampling, in the context of
  resolving the Hubble Tension. Working on those projects resulted in
  a custom fork of \texttt{ligra}, that performs 10x faster
  Bellman-Ford search on graphs with dynamic weights, and
  \texttt{supernest}, that made cosmological inference using
  \texttt{cobaya} 100 times faster, while also making it a 1000 times
  more precise.  Don't let the nominal amount of years fool you, I
  have been (competitively) programming in C++ since I was 12, and
  have a keen interest in \texttt{rust}, as a replacement for the
  venerable \texttt{FORTRAN} in scientific computing. I'm a team
  player, but currently confined to work remotely from Armenia, due to
  COVID. 
...
\newcommand{\eachpageornament}[1]{%
 ...
 {\setlength{\parindent}{0in}
   ...
   \job{Feb 2016}{Oct 2018} {KDE} {Qt Quick developer} {Developed of
     KDE Kube email client. Created the second largest amount of QA
     tests, helped implement the \term{imap} back-end. }
   ...
   \job{Oct 2018}{present} {SkyEng} {Tutor/Mathematical Corrector}
   {Online tutoring of upper-intermediate to proficiency students in
     ...
     like the \examtype{ielts,} \examtype{toefl} and \examtype{cae.}}
